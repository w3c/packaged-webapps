...
function buildTabs(products, assertions, tests, unknownProducts, unknownAssertions)
{
    ...
    if (count(unknownProducts.assertions) > 0)
        products['unknown-product'] = unknownProducts;
    // If there are unknown products add them to the array
    if (count(unknownAssertions) > 0)
        products['unknown-assertions'] = {id:'unknown-assertions', class:'',
    ...
    $('a.hash').click(function(){
        ...
        setTimeout(function(){parseHash($tabs)}, 100);
    });
}
    ...
    if (parts[1])
    {
        ...
        if (parts[2])
        {
            ...
            if (parts[2] != "tests")
            {
                ...
                $("html, body").scrollTop(ass.offset().top);
        }
    } else return;
...
function parseAssertions(spec, products)
{
        ...
        else
        {
            ...
            $prod.each(function(){
                ...
                if (prod)
                {
                    ...
                    if (prod.length === 0 || prod[prod.length-1].id != a.id)
                        prod.push(a);
    ...
    return {assertions: assertions, unknownProducts: unknownProducts};
}
